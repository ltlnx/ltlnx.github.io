...
function renderComment(comment, firstMentionFilter) {
  let result = document.querySelector('div.comments template').content.cloneNode(true);
...
function showComments(comments) {
  ...
  if(comments.length == 0) {
    ...
    contentElem.innerText = '目前無人回應。';
    commentsElem.classList.add('loaded', 'open');
    return;
  }
  // List of mentions that should be stripped if a top-level comment starts with one of them.
  // Insert your fedi ID here!
  let defaultInitialMentionFilter = ['https://g0v.social/@ltlnx'];
...
function loadComments(url) {
  ...
  fetch(url).then((response) => {
    ...
    console.error(err);
    document.querySelector('div.comments .placeholder').innerText = 'Could not load comments because of: ' + err.message; 
  });
}
...
function btnClick() {
  let commentsElem = document.querySelector('div.comments');
  let commentsBtn = document.querySelector('#comment-toggle');
  if (commentsElem.classList.contains('loaded')) {
    if (commentsElem.classList.contains('open')) {
      commentsElem.style.display = 'none';
      commentsBtn.innerHTML = "顯示留言";
      commentsElem.classList.remove('open');
    } else {
      commentsElem.style.display = 'block';
      commentsBtn.innerHTML = "隱藏留言";
      commentsElem.classList.add('open');
    }
  } else {
    initComments();
    commentsBtn.innerHTML = "隱藏留言";
